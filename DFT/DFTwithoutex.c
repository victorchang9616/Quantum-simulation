...
// global varables declartion 
float dxsav, *xp, *xb1, *xb2, **yp;
int kmax, kount;
int nrhs;                                                     
float delr = rmax / (float)N;             
float *VH, *u, E;
 ...
 hartreecal() 
{
	...
	for (int i = 0; i <= N; i++) 
	{
		VH[i] = 2.0 * (alpha*i*delr + VH[i] )/ (i * delr); 
	}
...
derivs(float x, float y[], float dydx[]) 
{
	...
	dydx[1] = y[2];
	int i = x / delr;         
	dydx[2] = 2.0*(-E - 2.0 / x + VH[i])*y[1];
}
int main()
{
	int nb;
	float root, zero, epsilon, Energy = 0.0, Energylocal;
	FILE *file = fopen("DFTwoexepi.txt", "w"); 
	FILE *file1 = fopen("DFTwoexE.txt", "w"); 
	xb1 = vector(1, 20);
	xb2 = vector(1, 20);
	u = vector(1, (N + 1));
	VH = vector(1, (N + 1));
	...
	zbrak(u0, -5.0, 2.0, 30, xb1, xb2, &nb);    // find root in [-5,1]              		
	epsilon = zbrent(u0, xb1[1], xb2[1], tol);
	printf("initial E=\n %10.6f\n", epsilon);
	zero = u0(epsilon);  // this is inital wave function stored in u[]
	E = epsilon;
	for (int i = 0; i < 250; i++) {
		hartreecal();
		zbrak(u0, -5.0, 2.0, 30, xb1, xb2, &nb);                		
		epsilon = zbrent(u0, xb1[1], xb2[1], tol);             
//		printf("for %dth iteration of xb1=%10.6f\nxb2=%10.6f\n", i, xb1[1],xb2[1]);
		zero = u0(epsilon);
		...
		// see if we meet the requirement
